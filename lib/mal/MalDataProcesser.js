...
class MalDataProcesser {
  ...
  processUserIdToLogin(userId, userLogin) {
    ...
    return this.db.tx((t) => {
      return this._userDuplicatesHandlerTx(t, userId, userLogin);
    });
  }
   ...
   *
   */
  _userDuplicatesHandlerTx(t, userId, userLogin) {
    return t.manyOrNone("\
      select id, login \
      from malrec_users \
        ...
        if (alreadyUserById && alreadyUserById.login != userLogin) {
          ...
          if (alreadyUserByLogin) {
            ...
            if (alreadyUserByLogin.id > 0) {
              ...
              this.redis.sadd("mal.userIdsToDel", alreadyUserByLogin.id);
            }
          } else {
  ...
  processProfile(userId, userLogin, user) {
    ...
    return this.db.tx((t) => {
      return this._userDuplicatesHandlerTx(t, newUserId, userLogin).then(() => {
        return t.oneOrNone("\
          select * \
          from malrec_users \
          where id = $(newUserId) \
