...
class MalDataProcesser {
  ...
  processAnime(animeId, anime) {
    ...
    if (Object.keys(anime.rels).length) {
      ...
      //Also skip "Character"
      let frRelTypes = ["Prequel", "Sequel", "Side story", "Parent story", 
        "Full story", "Summary", "Spin-off", "Alternative version", "Alternative setting"];
      let notFrRelTypes = ["Character", "Other"];
      let frRelAnimesIds = Object.keys(anime.rels)
        .filter((id) => (notFrRelTypes.indexOf(anime.rels[id]) == -1))
        .map(Number);
          ...
          this.db.tx((t) => {
            ...
            ").then((rows) => {
              ...
              return promise.then((frId) => {
                ...
                let idsWoFr = !rows ? [] : 
                  rows.filter(row => !row.franchise_id).map(row => row.id);
                if (!idsWoFr.length)
    ...
    return Promise.all(promises).then(() => {
      return this.db.tx((t) => {
        return Promise.all([
          ...
          if (anime.name)
            anime.name = anime.name.substring(0, 100);
          ...
          if (!oldAnime) {
            ...
            if (cols.length) {
              ...
              sql = "update malrec_items set " 
                + cols.map(c => c+' = '+'$('+c+')'+(c == 'genres' ? '::integer[]' : ''))
                    .join(", ")
                + " where id = $(id)";
            }
          }
  ...
  processProfile(userId, userLogin, user) {
    ...
    return this.db.tx((t) => {
      return this._handleUserDuplicatesTx(t, newUserId, userLogin).then(() => {
        return Promise.all([
          t.oneOrNone("\
            select id, trim(login) as login, gender, reg_date \
            from malrec_users \
            where id = $(newUserId) \
            for update \
          ", {
          ...
          t.oneOrNone("\
            select fav_items \
            from malrec_users_extra \
            where id = $(newUserId) \
          ", {
            newUserId: newUserId,
          }),
        ]).then(([oldUser, oldUserExtra]) => {
          let batch = [];
          let newUser = {
            id: newUserId,
            login: user.login,
          ...
          if (!oldUser) {
            ...
            cols = cols.filter((k) => (newUser[k] !== undefined && newUser[k] != oldUser[k]));
          ...
          if (sql != '')
            batch.push(t.query(sql, vals));

          //ins/upd user extra
          if (oldUserExtra === null && user.favs && user.favs.length || oldUserExtra !== null) {
            let vals = {}, cols = Object.keys(newUserExtra);
            let sql = "";
            if (!oldUserExtra) {
              vals = newUserExtra;
              sql = "insert into malrec_users_extra(" + cols.join(", ") + ")" 
                + " values(" + cols.map((k) => '$('+k+')').join(", ") + ")";
            } else {
              cols = cols.filter((k) => (k == 'fav_items' 
                ? !Helpers.isSameElementsInArrays(newUserExtra[k], oldUserExtra[k])
                : newUserExtra[k] !== undefined && newUserExtra[k] != oldUserExtra[k]));

              if (cols.length > 0) {
                vals = _.pick(newUserExtra, cols);
                vals.id = newUserId;
                sql = "update malrec_users_extra" 
                  + " set " + cols.map((k) => k+'='+'$('+k+')').join(", ")
                  + " where id = $(id)";
              }
            }
  ...
  processUserList(userId, userLogin, _listId, newList) {
    ...
    let isSafe = this.isSafeToModifyRatings();
    return this.db.tx((t) => {
      return this._handleUserDuplicatesTx(t, newUserId, userLogin).then(() => {
        // Get old list
        return Promise.all([
          ...
          //userRatingsChanges
          (isSafe ? Promise.resolve(null) : t.manyOrNone("\
            select item_id, rating \
            from malrec_ratings_changes \
            where user_list_id = \
              (select list_id from malrec_users where id = $(userId)) \
          ", {
            userId: newUserId,
          ...
          //userData
          t.oneOrNone("\
            select list_update_ts, list_check_ts, list_id \
            from malrec_users \
            where id = $(userId) \
          ", {
            userId: newUserId
        ...
        ]).then(([userRatings, userRatingsChanges, userData, userExtra]) => {
          if (!userData)
            throw new Error("User #"+newUserId + " "+userLogin + " not found in db");
          let userHasExtra = (userExtra !== null);
          let listId = userData.list_id;
          let oldList = { 
            userId: newUserId,
            ratings: {}, 
            unratedAnimeIdsInList: [], 
            listUpdatedTs: userData.list_update_ts,
            listCheckedTs: userData.list_check_ts,
          };
          if (userRatings)
            for (let row of userRatings) {
              oldList.ratings[row.item_id] = row.rating;
            }
          if (userRatingsChanges)
            for (let row of userRatingsChanges) {
              if (row.rating == 0)
                delete oldList.ratings[row.item_id];
              else
                oldList.ratings[row.item_id] = row.rating;
            }
          oldList.unratedAnimeIdsInList = userExtra && userExtra.unrated_items ? 
            userExtra.unrated_items : [];

          let hadList = Object.keys(oldList.ratings).length > 0;
          let hasList = Object.keys(newList.ratings).length > 0;
          let hasUnratedItems = newList.unratedAnimeIdsInList.length > 0;
          let oldListAnimeIds = Object.keys(oldList.ratings).map(Number);
          let newListAnimeIds = Object.keys(newList.ratings).map(Number);

          let promises = [];
          if (!listId && hasList) {
            let promGetList = t.query("\
              update malrec_users \
            ...
            promises.push(promGetList);
          }
          if (!userHasExtra && (hasList || hasUnratedItems)) {
            let promInsExtra = t.query("\
              insert into malrec_users_extra(id) \
              values ($(id)) \
            ", {
              id: newUserId,
            });
            promises.push(promInsExtra);
          }
          return Promise.all(promises).then(() => {
            if (hasList)
              assert(!!listId);
              ...
              if (unratedListChanged || ratsIdsToDel.length || Object.keys(ratsToAdd).length
               || Object.keys(ratsToUpd).length) {
                ...
                if (isSafe) {
                  ...
                  let newChanges = {};
                  for (let id of ratsIdsToDel)
                    newChanges[id] = 0;
                  for (let id in ratsToUpd)
                    newChanges[id] = ratsToUpd[id];
                  let newChangedItemIds = Object.keys(newChanges).map(Number);
                  let alreadyChangedItemIds = !userRatingsChanges ? [] : 
                    userRatingsChanges.map((row) => (row.item_id))
                    .filter((id) => (newChanges[id] !== undefined));
                  let notYetChangedItemIds = newChangedItemIds
                    .filter((id) => (alreadyChangedItemIds.indexOf(id) == -1));
                  for (let id of alreadyChangedItemIds) {
                    batch.push(t.query("\
                      update malrec_ratings_changes \
                      set rating = $(rating) \
                      where user_list_id = $(listId) and item_id = $(item_id)", {
                        rating: newChanges[id],
                        listId: listId,
                        item_id: id,
                      }));
                  }
                  if (notYetChangedItemIds.length) {
                    let iv = "";
                    let params = {};
                    params.listId = listId;
                    for (let id of notYetChangedItemIds) {
                      params['rat_'+id] = newChanges[id];
                      iv += (iv ? ", " : "") + "($(listId), " + id + ", $(rat_" + id + "))";
                    }
                    batch.push(t.query("\
                      insert into malrec_ratings_changes(user_list_id, item_id, rating) \
                      values " + iv, params));
                  }
                }
                ...
                if (Object.keys(ratsToAdd).length) {
                  ...
                  batch.push(t.query("\
                    insert into malrec_ratings(user_list_id, item_id, rating, dataset_type) \
                    values " + iv, params));
                }
