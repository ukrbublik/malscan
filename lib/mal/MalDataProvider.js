...
class MalDataProvider {
  ...
  getLoadUrlPromise (url, type = 'html', retry = -1) {
    let t1;
    return new Promise((resolve, reject) => {
      ...
      if (this.options.type == "webProxy") {
        if (this.options.webProxyType == 'Glype') {
          if (type == 'html') {
            ...
            reqConfig.headers = {
              Referer: this.options.addr + '/browse.php',
            }
          } else {
      ...
      request(reqConfig, (error, response, body) => {
        ...
        if (!error && !body)
          error = new MalError("Bad body: empty", response.statusCode, url, 3);
        if (!error && !this.isTrueMalSite(url, type, body))
          error = new MalError("Bad body: footer 'MyAnimeList' not found", 
            response.statusCode, url, 3);
        if (error && error.statusCode == 404) {
          if (this.isTrueMalSite(url, type, body))
            error.errCode = 2;
        }
        ...
        if (canRetry) {
          ...
          setTimeout(() => {
            this.getLoadUrlPromise(url, type, retry).then((body) => {
              resolve(body);
            }).catch((err) => {
              reject(err);
            });
   ...
   */
  isTrueMalSite(url, type = 'html', body) {
    let isHtml = (type == 'html'); // (body.indexOf("<html>") != -1)
    if (isHtml)
   ...
   *
   */
  loadUrl(url, type = 'html', retry = -1) {
    if (retry == -1)
      return this.queue.add(() => this.getLoadUrlPromise(url, type));
    else
      return this.getLoadUrlPromise(url, type, retry);
  }
  ...
  loadJson(url) {
    return this.loadUrl(url, 'json')
      ...
      .then((body) => {
        if (body.indexOf('<!DOCTYPE html') != -1 || body.indexOf('<!DOCTYPE HTML') != -1)
          throw new MalError("Bad result: HTML instead of JSON", 200, url, 3);
  ...
  loadXml(url) {
    return this.loadUrl(url, 'xml')
      ...
      .then((body) => {
        if (body.indexOf('<!DOCTYPE html') != -1 || body.indexOf('<!DOCTYPE HTML') != -1)
          throw new MalError("Bad result: HTML instead of XML", 200, url, 3);
        return new Promise((resolve, reject) => {
          ParseXmlString(body, {
            explicitArray: false
          }, (err, res) => {
        ...
        return new Promise((resolve, reject) => {
          ...
          ParseXmlString(body, {
            ...
            if (err)
              ...
              let channel = res.rss.channel;
              if (!channel)
                throw new MalError("Bad result: missing channel in RSS", 200, url, 3);
              if (channel.title == '')
                return null; //analog of 404
              else {
                let items = channel.item instanceof Array ? channel.item : [channel.item];
   ...
   *
   */
  fixHtmlBody($, body, pageUrl) {
    if (this.options.type == "webProxy") {
      if (this.options.webProxyType == 'Glype') {
        let links = $('a[href*="browse.php?u="]');
        let $a, url;
        for (let i = 0 ; i < links.length ; i++) {
          $a = $(links[i]);
          url = $a.attr('href');
          url = url.match(/browse\.php\?u=(.+?)(?:&b=|$)/)[1];
          if (!(url.indexOf("http") == 0 || url[0] == '/'))
            throw new MalError("Bad body: Web proxy had encypted links", 200, pageUrl, 3);
          url = decodeURIComponent(url);
          $a.attr('href', url);
        }
      } else if (this.options.webProxyType == 'PHProxy') {
        let links = $('a[href*="index.php?q="]');
        let $a, url;
        for (let i = 0 ; i < links.length ; i++) {
          $a = $(links[i]);
          url = $a.attr('href');
          url = url.match(/index\.php\?q=(.+)/)[1];
          if (!(url.indexOf("http") == 0 || url[0] == '/'))
            throw new MalError("Bad body: Web proxy had encypted links", 200, pageUrl, 3);
          url = decodeURIComponent(url);
          $a.attr('href', url);
  ...
  /**
   * @return null/{listUpdateDate: Date/null}
   */
   ...
   *  where relName = Other, Prequel, Sequel, Side story, Parent story, Full story,  
   *   Alternative version, Alternative setting, Spin-off, Summary, Character, ...
