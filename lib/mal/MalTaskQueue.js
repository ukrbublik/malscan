...
class MalTaskQueue {
  ...
  init(config) {
    ...
    .then(() => {
      ...
      for (let id in config.providers) {
        ...
        let provider = providerOptions.type == 'apiClient' ? new MalApiClient() 
          : new MalParser();
      ...
      return Promise.all(prvInitPromises).then(() => {
        ...
        for (let id in allProviders) {
          ...
          scnInitPromises.push(scanner.init(config, {}, this.db));
        }
  ...
  runTaskQueueLoop() {
    ...
    return Helpers.promiseWhile(() => !isNoMoreMoreTasks, () => {
      ...
      //Every iteration see if we need to recheck user ids/logins and do it immediately
      return Promise.all([
        this.redis.smembersAsync("mal.recheckUserIds"),
        this.redis.smembersAsync("mal.recheckUserLogins")
      ]).then(([userIds, userLogins]) => {
        if (userIds && userIds.length)
          this.redis.lpush("mal.queuedTasks", 'SpUserLogins_Re');
        else if (userLogins && userLogins.length)
          this.redis.lpush("mal.queuedTasks", 'UserProfiles_Re');
      }).then(() => {
        return this.redis.lpopAsync("mal.queuedTasks").then((nextTask) => {
          if (!nextTask)
            isNoMoreMoreTasks = true;
          else {
            let isKnownTask = (MalScanner.allTasksFuncs[nextTask] !== undefined);
            return (isKnownTask ? this.redis.setAsync("mal.processingTask", nextTask) 
              : Promise.reject("Unknown task " + nextTask))
            .then(() => this.performTask(nextTask))
            .catch((err) => {
              console.error("Error while performing task " + nextTask + ": ", err);
              return false;
            })
            .then((successed) => {
              let m = this.redis.multi();
              m.del("mal.processingTask");
              if (isKnownTask && !successed 
               && ['SpUserLogins_Re', 'UserProfiles_Re'].indexOf(nextTask) == -1)
    ...
    return MalScanner.shouldSkipTask(taskKey).then((shouldSkip) => {
      if (shouldSkip) {
        ...
        return Promise.resolve().then(() => {
          ...
          if (['GenresOnce'].indexOf(taskKey) != -1)
            maxProvidersCnt = 1; //use 1 scanner
          ...
          return MalScanner.beforeTask(taskKey, anyScanner).catch((err) => {
            anyScanner.isBusy = false;
            throw err;
          }).then((data) => {
            anyScanner.isBusy = false;

            if(data && data.listOfIds !== undefined) {
              if (data.listOfIds.length == 0) {
                console.log('> No ids in task ' + taskKey);
                return true;
              }
              maxProvidersCnt = Math.min(maxProvidersCnt, Math.ceil(data.listOfIds.length/100));
            }
            ...
            return Helpers.promiseWhile(() => !(isTaskSuccessed || isTaskFailed), () => {
              ...
              return new Promise((resolve, reject) => {
                tryCount++;
                console.log('> ' + (tryCount == 1 ? 'Starting' : 'Retrying') 
                  + ' task ' + taskKey);
                let i = 0;
                ...
                setTimeout(() => {
                  if (this.runningTasks[taskKey] !== undefined) {
                    let otherScannersIds = activeScannersIds
                      .filter((id) => (id != firstScannerId));
                ...
                taskEE.on("result", (id, res) => {
                  ...
                  if (activeScannersIds.length == 0) {
                    let lastRes = _.clone(res);
                    delete lastRes.errs;
                    console.log("> Result: ", lastRes);
                    if (isTaskSuccessed) {
                      //all good!
              ...
              return MalScanner.afterTask(taskKey, isTaskSuccessed, this.redis).then(() => {
                return isTaskSuccessed;
              });
            }).catch((err) => {
              delete this.runningTasks[taskKey];
              return MalScanner.afterTask(taskKey, false, this.redis).then(() => {
                throw err;
              });
            });
          });
    ...
    if (this.runningTasks[taskKey] !== undefined && !scanner.isBusy) {
      ...
      scanner.doGrabTask(taskKey, task.data).then((res) => {
        console.log("[" + id + "] " + "* " 
          ...
          + (res.errs && Object.keys(res.errs).length ? " with " + Object.keys(res.errs).length 
            + " errors" : ""));
        if (res.errs && Object.keys(res.errs).length)
  ...
  logGrabSpeed(taskKey, ext = false) {
    ...
    if (ext) {
      ...
      let cntIdsKey = 'mal.' + taskKey + '.cntTotalIds';
