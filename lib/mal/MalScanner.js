    ...
    return this.grabByIds({
      ...
      fetch: (id, data) => {
        ...
        if (0 && !mode != 'New' && mode != 'Need') {
          ...
          return (!data.listCheckedTs ? Promise.resolve(false) 
            : this.provider.getLastUserListUpdates({login: data.login}))
          .then((updatedRes) => {
            if (updatedRes === false || updatedRes !== null 
             && updatedRes.listUpdateDate && updatedRes.listUpdateDate > data.listCheckedTs) {
