...
class MalScanner extends MalBaseScanner {
  ...
  static get allTasksFuncs() {
    return {
      ...
      Animes_New: ['grabAnimes', 'New'],
      Animes_All: ['grabAnimes', 'All'],
  ...
  grabTest(data) {
    return this.grabByIds({
      ...
      getNextIds: (nextId, limit) => {
        let ids = Array.from({length: Math.min(limit, 15000 - nextId + 1)}, 
          (v, k) => nextId + k);
        return { ids: ids };
      },
   ...
   * 
   */
  grabAnimes(data, mode = 'New') {
    let key = 'Animes_'+mode;
    if (mode == 'New') {
      return this.grabByIds({
        key: key,
