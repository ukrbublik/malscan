 ...
 * Has basic thick function grabByIds() that manages grabbing in http queue 
 *  and safe parallel grabbing from many scanner insances (with help of redis).
 * Scanner instances can even run together from many processes and PCs. 
 *
 * @author ukrbublik
 */
...
class MalBaseScanner {
  ...
  init(config, options = {}, dbConnection = null) {
    ...
    return Promise.all([
      ...
      this.checkDbConnection().then(() => {
        return this.processer.init(this.options.processer, this.db, this.redis);
      }),
    ]);
  }
  ...
  /**
   * @param object opts - assoc array with keys:
   ...
   *  getNextIds (for isByListOfIds==false) - optional, function(nextId, limit) returning 
   *   promise to get next ids to grab and corresponding data (logins for users), 
   *   returns {ids: <arr>, data: <obj>}
  ...
  grabByIds(opts) {
    ...
    let progressSavedEventKey = 'progressSaved_' + opts.key;
    ...
    let progressIsSaving = false;
    let waitingForProgressSaved = false;
      ...
      return this.redis.mgetAsync([nextIndKey, grabIterKey, 
        ...
        if (rNextInd === null) {
          if (rRetryIds.length) {
            ...
            let skipTimeIntrv = 1000*60*5;
            if (['UserProfiles_Re', 'SpUserLogins_Re'].indexOf(opts.key) != -1)
              skipTimeIntrv = 1000*10; //10s
            if (rDateIterEnd && (new Date().getTime() - rDateIterEnd) < skipTimeIntrv)
              return Promise.resolve(false);
            else return Promise.resolve().then(() => opts.getTotalIdsCnt())
            .then((totalIdsCnt) => {
              let multi = this.redis.multi();
    ...
    let grabLostOrNext = () => {
      ...
      if (progressIsSaving) {
        waitingForProgressSaved = true;
        return new Promise((resolve, reject) => {
          this.eventEmitter.once(progressSavedEventKey, () => {
            waitingForProgressSaved = false;
            grabLostOrNext().then((res) => { resolve(res); });
          });
        });
      }

      grabbingNext = true;
      progressIsSaving = true;
      let pr;
      ...
      return pr.then((pickedIdsCnt) => {
        ...
        grabbingNext = false;
        progressIsSaving = false;
        if (waitingForProgressSaved)
          this.eventEmitter.emit(progressSavedEventKey);
        return pickedIdsCnt;
      });
    };
      ...
      return this.redis.mgetAsync([nextIndKey, grabIterKey])
        .then(([rNextInd, rGrabIter]) => {
          ...
          return getResPromise.then((res) => {
            ...
            // They must be in 1 transaction! Also see saveProgress()
    ...
    let saveProgress = (retry = -1) => {
      if (retry == -1 && progressIsSaving) {
        waitingForProgressSaved = true;
        return new Promise((resolve, reject) => {
          this.eventEmitter.once(progressSavedEventKey, () => {
            waitingForProgressSaved = false;
            saveProgress().then((res) => { resolve(res); });
          });
        });
      }
      ...
      return multi.execAsync().then((replies) => {
        if (replies === null) {
          if (_needToSaveErrorIds)
            needToSaveErrorIds = true;       
          return saveProgress(retry + 1); //retry
        } else {
          incrCntSuccessIds -= _incrCntSuccessIds;
          incrCntNotFoundIds -= _incrCntNotFoundIds;
          progressIsSaving = false;
    ...
    .then((started) => {
      if (!started)
        ...
        .then((res) => {
          ...
          this.eventEmitter.removeAllListeners(progressSavedEventKey);
          return res;
        }).catch((err) => {
          this.dateStart = null;
          this.eventEmitter.removeAllListeners(grabCompleteEventKey);
