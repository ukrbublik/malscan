/**
 * Client for MAL API (unofficial) server
 *
 * @author ukrbublik
 */
...
class MalApiClient extends MalDataProvider {
  ...
  getLastUserListUpdates({login}) {
    return this.queryMethod('getLastUserListUpdates', {login: login}).then((res) => {
      if (res && res.listUpdateDate)
        res.listUpdateDate = new Date(res.listUpdateDate);
      return res;
    });
  }
