...
class MalParser extends MalDataProvider {
  ...
  getLastUserListUpdates({login}) {
    ...
    return this.loadRss(url).then((items) => {
      if (items === null)
        return null; //No user
      let listUpdateDate = items.length ? items[0].pubDate : null;
      return {
  ...
  getProfileInfo({login}) {
    ...
    return this.loadHtml(url).then(($) => {
      ...
      if (genderStr)
        user.gender = genderStr; //Male, Female, Non-Binary
    ...
    if (!altJson) {
      ...
      return this.loadXml(url).then((res) => {
        if (res.myanimelist === undefined) {
          ...
          let userInfo = res.myanimelist.myinfo ? res.myanimelist.myinfo : null;
          let animeList = res.myanimelist.anime ? res.myanimelist.anime : null;
          if (animeList !== null)
            animeList = (animeList instanceof Array ? animeList : [animeList]);
          if (!userInfo) {
            return null;
          } else {
            let userId = parseInt(userInfo.user_id);
            let ratings = {};
            ...
            if (animeList) {
              for (let item of animeList) {
                ...
                let rating = parseInt(item.my_score);
                let updated = parseInt(item.my_last_updated);
